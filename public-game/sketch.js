...
socket.on('buttonBPressed', function(data) {
    // Aumenta la velocidad de la pelota
    ballSpeed += data.increaseBy;
  });
  
  // Define la velocidad inicial de la pelota
  let ballSpeed = 1;
  
  // Define la función que dibuja la pelota
  function drawBall() {
    // Código para dibujar la pelota
    // ...
    
    // Actualiza la posición de la pelota según la velocidad
    ballX += ballSpeed;
  }
  
  // Define la función que se ejecuta cuando se presiona el botón B
  function buttonBPressed() {
    // Envía un mensaje al servidor indicando que se ha presionado el botón B
    socket.emit('buttonBPressed', { increaseBy: 0.5 });
  }
  
  // Agrega un event listener al botón B para detectar cuando se presiona
  document.getElementById('buttonB').addEventListener('click', buttonBPressed);
...
// Define la función que dibuja el jugador
function drawPlayer() {
  // Código para dibujar el jugador con la anchura actual
  // ...
}

// Define la función que se ejecuta cuando se presiona el botón A
function buttonAPressed() {
  // Reducir la anchura del jugador en 10 píxeles
  playerWidth -= 10;
}

// Agrega un event listener al botón A para detectar cuando se presiona
document.getElementById('buttonA').addEventListener('click', buttonAPressed);
